...
// For the closing of procedure declaration
...
void Sema::actOnProcedureCall(StmtList &Stmts, SMLoc Loc, Decl *D,
                              ExprList &Params) {
...
void Sema::actOnIfStatement(StmtList &Stmts, SMLoc Loc, Expr *Cond,
  ...
  // an if without cond never executes
  if (!Cond) {
    Cond = FalseLiteral;
  }

  // only booleans as condition exprs
...
void Sema::actOnReturnStatement(StmtList &Stmts, SMLoc Loc, Expr *RetVal) {
  // why cast and not dyn_cast_or_null? no returns outside procs according to
  // grammar.
...
Expr *Sema::actOnSimpleExpression(Expr *Left, Expr *Right,
  ...
  bool IsConst = Left->isConstant() && Right->isConstant();
  // evaluate constants during semantic analysis
...
Expr *Sema::actOnPrefixExpression(Expr *E, const OperatorInfo &Op) {
  ...
  if (E->isConstant() && Op.getKind() == tok::kw_NOT) {
